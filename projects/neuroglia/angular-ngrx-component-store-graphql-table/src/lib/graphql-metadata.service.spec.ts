...
describe('GraphQL Metadata Service', () => {
  let service: GraphQLMetadataService;
  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [provideHttpClient(), NamedLoggingServiceFactory, HttpErrorObserverService, UrlHelperService],
    });
    service = TestBed.inject(GraphQLMetadataService);
  });

  describe('getMetadataFromIntrospection', () => {
    it('should build schema from introspection query', (done) => {
      service.getMetadataFromIntrospection(testEndpoint).subscribe({
        next: (schema) => {
      ...
      service
        ...
        .subscribe({
          next: (definitions: ColumnDefinition[]) => {
            ...
            expect(definitions.length).toBe(expectedColumnNames.length);
            expect(definitions.map((def) => def.name).toSorted()).toEqual(expectedColumnNames);
            done();
          },
          error: (err) => {
            expect(err).withContext('error').toBeNull();
