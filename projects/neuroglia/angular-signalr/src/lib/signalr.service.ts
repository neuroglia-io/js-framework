...
export abstract class SignalRService implements OnDestroy {
  ...
  connect(
    ...
    if (!withAutomaticReconnect) {
      retryStrategy = (attemps: Observable<Error>): Observable<number> =>
        attemps.pipe(mergeMap((error) => throwError(error)));
    } else {
      ...
      const nextRetryDelayInMilliseconds: (retryContext: signalR.RetryContext) => number | null =
    ...
    const repeatableConnect = new Observable<void>((subscribe) => {
      ...
      this.connection.start().then(subscribe.next.bind(subscribe)).catch(subscribe.error.bind(subscribe));
    });
    return logHttpRequest<void>(this.logger, this.errorObserver, repeatableConnect, httpRequestInfo).pipe(
  ...
  send(method: string, ...args: any[]): Observable<void> {
    ...
    const httpRequestInfo: HttpRequestInfo = new HttpRequestInfo({
      clientServiceName: 'SignalRService',
      methodName: `send '${method}'`,
      verb: 'websocket',
      url: this.url,
      postData: JSON.stringify(args),
    });
    return logHttpRequest<void>(
      this.logger,
      this.errorObserver,
   ...
   */
  ngOnDestroy() {
    this.disconnect().subscribe().unsubscribe();
  }
